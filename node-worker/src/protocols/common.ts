...
import { isIPv4, parseHostPort, resolveDNS } from '@cores/utils';
...
export async function handleTCPOutBound(
    ...
    async function connectAndWrite(address: string, port: number): Promise<Socket> {
        ...
        writer.releaseLock();
        // client -> upstream (tunnel leg): count once, here, where the tunnel
        // finally emits the payload upstream (Document 3, §K: only the relay
        // code ever sees the real byte counts)
